%
% CODE BELOW SLIGHTLY EDITED BY DOBY RAHNEV.
% IF REUSING, IT'S HIGHLY RECOMMENDED THAT YOU USE THE ORIGINAL FILES.
%
...
function [h, u] = raincloud_plot_doby(X, varargin)
...
if box_on
    ...
    if box_dodge
        width = .15;
        box_pos = [Y(1), -box_dodge_amount-width/2, Y(2)-Y(1), width];
        
        % mean line
        h{4} = line([Y(3) Y(3)], [-box_dodge_amount - width/2, -box_dodge_amount + width/2], 'col', bxcl, 'LineWidth', line_width);
        
        % whiskers
        h{5} = line([Y(2) Y(5)], [-box_dodge_amount, -box_dodge_amount], 'col', bxcl, 'LineWidth', line_width);
        h{6} = line([Y(1) Y(4)], [-box_dodge_amount, -box_dodge_amount], 'col', bxcl, 'LineWidth', line_width);
    else
